...
def _pick_route(center_lat: float, center_lon: float, max_dist_km: float = 300,
                waypoints: Optional[list[tuple[float, float]]] = None) -> list[tuple[float, float]]:
...
class SimulationWorld:
    ...
    def __init__(self, center_lat: float = 34.85, center_lon: float = -82.39,
    ...
    def _nationwide_pose(self) -> tuple[float, float, list]:
        ...
        far = [wp for wp in net
               if _haversine_km(start[0], start[1], wp[0], wp[1]) > min_leg]
